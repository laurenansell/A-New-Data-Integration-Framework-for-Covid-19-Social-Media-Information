...
# read in the data
UK_data <- read_csv("marginal_data_Com.csv")
#
#
UK_data_sel <- UK_data %>% dplyr::select(date,hits, newVirusTests, newAdmissions, hospitalCases,
                                         covidOccupiedMVBeds, count,deaths_count,Bing,Afinn,Total_tweets)
UK_data_sel$date<-as.Date(UK_data_sel$date,format="%d/%m/%Y")
#
### rename variables
#
UK_data_ren <- UK_data_sel %>% rename(Google=hits,Cases=count)
#
# create time series object
UK_data_ts <- UK_data_ren[,-1] %>% as_tibble() %>% ts(start=c(2020), frequency=365.25)
#
### Turn the data into a tsibble object
#
UK_data_tsi <- UK_data_ren %>% tsibble::as_tsibble(index = date)
#
#
## Fit the models
##
...
hist(u.VT,main="Histogram of the Virus Tests u data")
hist(u.TW,main="Histogram of the Total Tweets u data")
...
rv<-vinecop(u,family_set="par",presel=FALSE)

# Contour plots
contour(rv,cex.nums=.8)
# Looks like a few independence copulas, particularly in the lower trees.

# Fit results
sum.rv<- summary(rv)
print(as.data.frame(sum.rv), digits = 2)

## Lets look at the trees

# Tree 1
plot(rv,tree=1,edge_labels="family_tau",var_names="use")
# Tree 2
plot(rv,tree=2,edge_labels="family_tau",var_names="use") # see first independence copula here
# Tree 3
plot(rv,tree=3,edge_labels="family_tau",var_names="use")
# Tree 4
plot(rv,tree=4,edge_labels="family_tau",var_names="use")
# Tree 5
plot(rv,tree=5,edge_labels="family_tau",var_names="use")
# Tree 6
plot(rv,tree=6,edge_labels="family_tau",var_names="use")
# Tree 7
plot(rv,tree=7,edge_labels="family_tau",var_names="use")
# Tree 8
plot(rv,tree=8,edge_labels="family_tau",var_names="use")
# Tree 9
plot(rv,tree=9,edge_labels="family_tau",var_names="use")
## So may be we want to set a threshold?

## D vine

set.seed(2)
udata.cop<-as.copuladata(u)
M = 1 - abs(TauMatrix(udata.cop))
hamilton = insert_dummy(TSP(M),label="cut")
sol = solve_TSP(hamilton,method="repetitive_nn")
order = cut_tour(sol,"cut")
dv<-vinecop(u,structure=dvine_structure(order),
            family_set="par",presel=FALSE)

# Contour plots
contour(dv,cex.nums=.8)

# Fit results
sum.dv<-summary(dv)
print(as.data.frame(sum.dv), digits = 2)

## Lets look at the trees

# Tree 1
plot(dv,tree=1,edge_labels="family_tau",var_names="use")
# Tree 2
plot(dv,tree=2,edge_labels="family_tau",var_names="use") # see first independence copulas here
# Tree 3
plot(dv,tree=3,edge_labels="family_tau",var_names="use") 
# Tree 4
plot(dv,tree=4,edge_labels="family_tau",var_names="use") 
# Tree 5
plot(dv,tree=5,edge_labels="family_tau",var_names="use")
# Tree 6
plot(dv,tree=6,edge_labels="family_tau",var_names="use")
# Tree 7
plot(dv,tree=7,edge_labels="family_tau",var_names="use")
# Tree 8
plot(dv,tree=8,edge_labels="family_tau",var_names="use")
# Tree 9
plot(dv,tree=9,edge_labels="family_tau",var_names="use")

## C vine

cv<-RVineStructureSelect(udata.cop, family=NA,
                         method = "mle",type="CVine")

# Contour plots
contour(cv,cex.nums = .8)

# Fit results
sum.cv<-summary(cv)
print(as.data.frame(sum.cv),digits=2)
...
order.cv<-cv$Matrix[,1]

cv1<-vinecop(u,presel=FALSE,structure=cvine_structure(order.cv),
             family_set="par")

## Comparison of the vines

vine.out<-function(fit=rv,data=u,digits=2){
  out<-round(c(fit$loglik,fit$npars,AIC(fit),BIC(fit),
               mBICV(fit)),digits)
  names(out)<-c("loglik","par","AIC","BIC","mBICV")
  out
}
vine.out(rv,u)
vine.out(dv,u)
vine.out(cv1,udata.cop)
...
# Tree 1
plot(rv1,tree=1,edge_labels="family_tau",var_names="use")
# Tree 2
plot(rv1,tree=2,edge_labels="family_tau",var_names="use") 
# Tree 3
plot(rv1,tree=3,edge_labels="family_tau",var_names="use") # see first independence copula here
# Tree 4
plot(rv1,tree=4,edge_labels="family_tau",var_names="use")
# Tree 5
plot(rv1,tree=5,edge_labels="family_tau",var_names="use") 
...
sol = solve_TSP(hamilton,method="repetitive_nn")
order = cut_tour(sol,"cut")
dv1<-vinecop(u,structure=dvine_structure(order),
             family_set="par",presel=TRUE)

# Contour plots
contour(dv1,cex.nums=.8)

# Fit results
sum.dv1<-summary(dv1)
print(as.data.frame(sum.dv1), digits = 2)

## Lets look at the trees

# Tree 1
plot(dv1,tree=1,edge_labels="family_tau",var_names="use")
# Tree 2
plot(dv1,tree=2,edge_labels="family_tau",var_names="use") 
# Tree 3
plot(dv1,tree=3,edge_labels="family_tau",var_names="use") # see first independence copula here
# Tree 4
plot(dv1,tree=4,edge_labels="family_tau",var_names="use")
# Tree 5
plot(dv1,tree=5,edge_labels="family_tau",var_names="use") 


## C vine

cv_1<-RVineStructureSelect(udata.cop, family=NA,
                           method = "mle",type="CVine")

# Contour plots
contour(cv_1,cex.nums = .8)

# Fit results
sum.cv_1<-summary(cv_1)
print(as.data.frame(sum.cv_1),digits=2)
...
order.cv_1<-cv_1$Matrix[,1]

cv_11<-vinecop(u,presel=TRUE,structure=cvine_structure(order.cv_1),
               family_set="par")

contour(cv_11,cex.nums = .8)

# Fit results
sum.cv_11<-summary(cv_11)
print(as.data.frame(sum.cv_11),digits=2)

## Lets look at the trees

# Tree 1
plot(cv_11,tree=1,edge_labels="family_tau",var_names="use")
# Tree 2
plot(cv_11,tree=2,edge_labels="family_tau",var_names="use")
# Tree 3
plot(cv_11,tree=3,edge_labels="family_tau",var_names="use")
# Tree 4
plot(cv_11,tree=4,edge_labels="family_tau",var_names="use")
# Tree 5
plot(cv_11,tree=5,edge_labels="family_tau",var_names="use")
...
## Google
#
UK_predplot <- ggplot(data=UK_data_ren,aes(x = date, y = Google)) + 
  geom_line() + 
  geom_line(data=UK_sim_avg, aes(x = Dates_test, y = Google), col="red") +
...
## Twitter
#
UK_predplot1 <- ggplot(data=UK_data_ren,aes(x = date, y = Total_tweets)) + 
  geom_line() + 
  geom_line(data=UK_sim_avg, aes(x = Dates_test, y = Twitter), col="red") +
...
## Bing
#
UK_predplot2 <- ggplot(data=UK_data_ren,aes(x = date, y = Bing)) + 
  geom_line() + 
  geom_line(data=UK_sim_avg, aes(x = Dates_test, y = Bing), col="red") +
  ggtitle("Bing") +
  xlab("Date") +
  ylab("Bing Score")
UK_predplot2
...
## Afinn
#
UK_predplot3 <- ggplot(data=UK_data_ren,aes(x = date, y = Afinn)) + 
  geom_line() + 
  geom_line(data=UK_sim_avg, aes(x = Dates_test, y = Afinn), col="red") +
  ggtitle("Afinn") +
  xlab("Date") +
  ylab("Afinn Score")
UK_predplot3
...
## Deaths
#
UK_predplot4 <- ggplot(data=UK_data_ren,aes(x = date, y = deaths_count)) + 
  geom_line() + 
  geom_line(data=UK_sim_avg, aes(x = Dates_test, y = Deaths), col="red") +
...
## Hospital Cases
#
UK_predplot5 <- ggplot(data=UK_data_ren,aes(x = date, y = hospitalCases)) + 
  geom_line() + 
  geom_line(data=UK_sim_avg, aes(x = Dates_test, y = Hospital_cases), col="red") +
  ggtitle("Hospital cases") +
  xlab("Date") +
  ylab("Hospital Cases")
UK_predplot5
#
## New Admissions
#
UK_predplot6 <- ggplot(data=UK_data_ren,aes(x = date, y = newAdmissions)) + 
  geom_line() + 
  geom_line(data=UK_sim_avg, aes(x = Dates_test, y = New_Admissions), col="red") +
  ggtitle("New admissions") +
  xlab("Date") +
  ylab("New admissions")
UK_predplot6
#
## Cases
#
UK_predplot7 <- ggplot(data=UK_data_ren,aes(x = date, y = Cases)) + 
  geom_line() + 
  geom_line(data=UK_sim_avg, aes(x = Dates_test, y = New_Cases), col="red") +
  ggtitle("Cases") +
  xlab("Date") +
  ylab("Cases")
UK_predplot7
...
## Occupied Beds
#
UK_predplot8 <- ggplot(data=UK_data_ren,aes(x = date, y = covidOccupiedMVBeds)) + 
  geom_line() + 
  geom_line(data=UK_sim_avg, aes(x = Dates_test, y = Occupied_Beds), col="red") +
  ggtitle("Occupied Beds") +
  xlab("Date") +
  ylab("Occupied Beds")
UK_predplot8
#
## Virus Tests
#
UK_predplot9 <- ggplot(data=UK_data_ren,aes(x = date, y = VTadj)) + 
  geom_line() + 
  geom_line(data=UK_sim_avg, aes(x = Dates_test, y = Virus_Tests), col="red") +
  ggtitle("Virus Tests") +
  xlab("Date") +
  ylab("Virus tests per 1000")
UK_predplot9
#
#
#
...
## Google
#
UK_predplot_G <- ggplot(data=UK_data_ren,aes(x = date, y = Google)) + 
  geom_line() + 
  geom_line(data=UK_sim_G_avg, aes(x = Dates_test, y = Google), col="red") +
  ggtitle("Google trends") +
  xlab("Date") +
  ylab("number of searches")
UK_predplot_G
...
## Twitter
#
UK_predplot1_G <- ggplot(data=UK_data_ren,aes(x = date, y = Total_tweets)) + 
  geom_line() + 
  geom_line(data=UK_sim_G_avg, aes(x = Dates_test, y = Twitter), col="red") +
  ggtitle("Total Tweets") +
  xlab("Date") +
  ylab("number of Tweets")
UK_predplot1_G
...
## Bing
#
UK_predplot2_G <- ggplot(data=UK_data_ren,aes(x = date, y = Bing)) + 
  geom_line() + 
  geom_line(data=UK_sim_G_avg, aes(x = Dates_test, y = Bing), col="red") +
  ggtitle("Bing") +
  xlab("Date") +
  ylab("Bing Score")
UK_predplot2_G
...
## Afinn
#
UK_predplot3_G <- ggplot(data=UK_data_ren,aes(x = date, y = Afinn)) + 
  geom_line() + 
  geom_line(data=UK_sim_G_avg, aes(x = Dates_test, y = Afinn), col="red") +
  ggtitle("Afinn") +
  xlab("Date") +
  ylab("Afinn Score")
UK_predplot3_G
...
## Deaths
#
UK_predplot4_G <- ggplot(data=UK_data_ren,aes(x = date, y = deaths_count)) + 
  geom_line() + 
  geom_line(data=UK_sim_G_avg, aes(x = Dates_test, y = Deaths), col="red") +
  ggtitle("Number of deaths") +
  xlab("Date") +
  ylab("Number of deaths")
UK_predplot4_G
...
## Hospital Cases
#
UK_predplot5_G <- ggplot(data=UK_data_ren,aes(x = date, y = hospitalCases)) + 
  geom_line() + 
  geom_line(data=UK_sim_G_avg, aes(x = Dates_test, y = Hospital_cases), col="red") +
  ggtitle("Hospital cases") +
  xlab("Date") +
  ylab("Hospital Cases")
UK_predplot5_G
...
## New Admissions
#
UK_predplot6_G <- ggplot(data=UK_data_ren,aes(x = date, y = newAdmissions)) + 
  geom_line() + 
  geom_line(data=UK_sim_G_avg, aes(x = Dates_test, y = New_Admissions), col="red") +
  ggtitle("New admissions") +
  xlab("Date") +
  ylab("New admissions")
UK_predplot6_G
...
## Cases
#
UK_predplot7_G <- ggplot(data=UK_data_ren,aes(x = date, y = Cases)) + 
  geom_line() + 
  geom_line(data=UK_sim_G_avg, aes(x = Dates_test, y = New_Cases), col="red") +
  ggtitle("Cases") +
  xlab("Date") +
  ylab("Cases")
UK_predplot7_G
...
## Occupied Beds
#
UK_predplot8_G <- ggplot(data=UK_data_ren,aes(x = date, y = covidOccupiedMVBeds)) + 
  geom_line() + 
  geom_line(data=UK_sim_G_avg, aes(x = Dates_test, y = Occupied_Beds), col="red") +
  ggtitle("Occupied Beds") +
  xlab("Date") +
  ylab("Occupied Beds")
UK_predplot8_G
...
## Virus Tests
#
UK_predplot9_G <- ggplot(data=UK_data_ren,aes(x = date, y = VTadj)) + 
  geom_line() + 
  geom_line(data=UK_sim_G_avg, aes(x = Dates_test, y = Virus_Tests), col="red") +
  ggtitle("Virus Tests") +
  xlab("Date") +
  ylab("Virus tests per 1000")
UK_predplot9_G
...
## Google
#
UK_predplot_I<- ggplot(data=UK_data_ren,aes(x = date, y = Google)) + 
  geom_line() + 
  geom_line(data=UK_sim_I_avg, aes(x = Dates_test, y = Google), col="red") +
  ggtitle("Google trends") +
  xlab("Date") +
  ylab("number of searches")
UK_predplot_I
...
## Twitter
#
UK_predplot1_I <- ggplot(data=UK_data_ren,aes(x = date, y = Total_tweets)) + 
  geom_line() + 
  geom_line(data=UK_sim_I_avg, aes(x = Dates_test, y = Twitter), col="red") +
  ggtitle("Total Tweets") +
  xlab("Date") +
  ylab("number of Tweets")
UK_predplot1_I
...
## Bing
#
UK_predplot2_I <- ggplot(data=UK_data_ren,aes(x = date, y = Bing)) + 
  geom_line() + 
  geom_line(data=UK_sim_I_avg, aes(x = Dates_test, y = Bing), col="red") +
  ggtitle("Bing") +
  xlab("Date") +
  ylab("Bing Score")
UK_predplot2_I
...
## Afinn
#
UK_predplot3_I <- ggplot(data=UK_data_ren,aes(x = date, y = Afinn)) + 
  geom_line() + 
  geom_line(data=UK_sim_I_avg, aes(x = Dates_test, y = Afinn), col="red") +
  ggtitle("Afinn") +
  xlab("Date") +
  ylab("Afinn Score")
UK_predplot3_I
...
## Deaths
#
UK_predplot4_I <- ggplot(data=UK_data_ren,aes(x = date, y = deaths_count)) + 
  geom_line() + 
  geom_line(data=UK_sim_I_avg, aes(x = Dates_test, y = Deaths), col="red") +
  ggtitle("Number of deaths") +
  xlab("Date") +
  ylab("Number of deaths")
UK_predplot4_I
...
## Hospital Cases
#
UK_predplot5_I <- ggplot(data=UK_data_ren,aes(x = date, y = hospitalCases)) + 
  geom_line() + 
  geom_line(data=UK_sim_I_avg, aes(x = Dates_test, y = Hospital_cases), col="red") +
  ggtitle("Hospital cases") +
  xlab("Date") +
  ylab("Hospital Cases")
UK_predplot5_I
...
## New Admissions
#
UK_predplot6_I <- ggplot(data=UK_data_ren,aes(x = date, y = newAdmissions)) + 
  geom_line() + 
  geom_line(data=UK_sim_I_avg, aes(x = Dates_test, y = New_Admissions), col="red") +
  ggtitle("New admissions") +
  xlab("Date") +
  ylab("New admissions")
UK_predplot6_I
...
## Cases
#
UK_predplot7_I <- ggplot(data=UK_data_ren,aes(x = date, y = Cases)) + 
  geom_line() + 
  geom_line(data=UK_sim_I_avg, aes(x = Dates_test, y = New_Cases), col="red") +
  ggtitle("Cases") +
  xlab("Date") +
  ylab("Cases")
UK_predplot7_I
...
## Occupied Beds
#
UK_predplot8_I <- ggplot(data=UK_data_ren,aes(x = date, y = covidOccupiedMVBeds)) + 
  geom_line() + 
  geom_line(data=UK_sim_I_avg, aes(x = Dates_test, y = Occupied_Beds), col="red") +
  ggtitle("Occupied Beds") +
  xlab("Date") +
  ylab("Occupied Beds")
UK_predplot8_I
...
## Virus Tests
#
UK_predplot9_I <- ggplot(data=UK_data_ren,aes(x = date, y = VTadj)) + 
  geom_line() + 
  geom_line(data=UK_sim_I_avg, aes(x = Dates_test, y = Virus_Tests), col="red") +
  ggtitle("Virus Tests") +
  xlab("Date") +
  ylab("Virus tests per 1000")
UK_predplot9_I
...
### Google trends
#
G_pred_int_M<-as.data.frame(test_Go[1:37,])
G_pred_int_M<-cbind(G_pred_int_M,lower_M_Go,upper_M_Go)
#
UK_predintplot_M_G <- ggplot(G_pred_int_M,aes(x = date, y = Google)) + 
  geom_line() + 
  geom_line(data=UK_sim_avg, aes(x = Dates_test, y = Google), col="red") +
  geom_line(data=G_pred_int_M,aes(x=date,y=lower_M_Go),col="red")+
  geom_line(data=G_pred_int_M,aes(x=date,y=upper_M_Go),col="red")+
  ggtitle("Google trends prediction interval with Mixed Copula") +
  xlab("Date") +
  ylab("number of searches")
UK_predintplot_M_G
...
### Twitter
#
Tw_pred_int_M<-as.data.frame(test_Tw[1:37,])
Tw_pred_int_M<-cbind(Tw_pred_int_M,lower_M_TW,upper_M_TW)
#
UK_predintplot_M_Tw <- ggplot(Tw_pred_int_M,aes(x = date, y = Total_tweets)) + 
  geom_line() + 
  geom_line(data=UK_sim_avg, aes(x = Dates_test, y = Twitter), col="red") +
  geom_line(data=Tw_pred_int_M,aes(x=date,y=lower_M_TW),col="red")+
  geom_line(data=Tw_pred_int_M,aes(x=date,y=upper_M_TW),col="red")+
  ggtitle("Twitter search prediction interval with Mixed Copula") +
  xlab("Date") +
  ylab("number of searches")
UK_predintplot_M_Tw
...
### Virus Tests
#
VT_pred_int_M<-as.data.frame(test_VT[1:37,])
VT_pred_int_M<-cbind(VT_pred_int_M,lower_M_VT,upper_M_VT)
#
UK_predintplot_M_VT <- ggplot(VT_pred_int_M,aes(x = date, y = VTadj)) + 
  geom_line() + 
  geom_line(data=UK_sim_avg, aes(x = Dates_test, y = Virus_Tests), col="red") +
  geom_line(data=VT_pred_int_M,aes(x=date,y=lower_M_VT),col="red",linetype=2)+
  geom_line(data=VT_pred_int_M,aes(x=date,y=upper_M_VT),col="red",linetype=2)+
  ggtitle("Virus Test (per 1000) prediction interval with Mixed Copula") +
  xlab("Date") +
  ylab("Number of Virus Tests")+
...
### New Admissions
#
NA_pred_int_M<-as.data.frame(test_NA[1:37,])
NA_pred_int_M<-cbind(NA_pred_int_M,lower_M_NA,upper_M_NA)
#
UK_predintplot_M_NA <- ggplot(NA_pred_int_M,aes(x = date, y = newAdmissions)) + 
  geom_line() + 
  geom_line(data=UK_sim_avg, aes(x = Dates_test, y = New_Admissions), col="red") +
  geom_line(data=NA_pred_int_M,aes(x=date,y=lower_M_NA),col="red",linetype=2)+
  geom_line(data=NA_pred_int_M,aes(x=date,y=upper_M_NA),col="red",linetype=2)+
  ggtitle("New Admissions prediction interval with Mixed Copula") +
  xlab("Date") +
  ylab("Number of New Admissions")+
  theme(axis.text=element_text(size=20),
        axis.title=element_text(size=20),
        title = element_text(size=18))
UK_predintplot_M_NA
...
### Bing
#
Bi_pred_int_M<-as.data.frame(test_Bing[1:37,])
Bi_pred_int_M<-cbind(Bi_pred_int_M,lower_M_Bi,upper_M_Bi)
#
UK_predintplot_M_Bi <- ggplot(Bi_pred_int_M,aes(x = date, y = Bing)) + 
  geom_line() + 
  geom_line(data=UK_sim_avg, aes(x = Dates_test, y = Bing), col="red") +
  geom_line(data=Bi_pred_int_M,aes(x=date,y=lower_M_Bi),col="red")+
  geom_line(data=Bi_pred_int_M,aes(x=date,y=upper_M_Bi),col="red")+
  ggtitle("Bing score prediction interval with Mixed Copula") +
  xlab("Date") +
  ylab("Bing sentiment score")
UK_predintplot_M_Bi

### Afinn
#
Af_pred_int_M<-as.data.frame(test_Afinn[1:37,])
Af_pred_int_M<-cbind(Af_pred_int_M,lower_M_Af,upper_M_Af)
#
UK_predintplot_M_Af <- ggplot(Af_pred_int_M,aes(x = date, y = Afinn)) + 
  geom_line() + 
  geom_line(data=UK_sim_avg, aes(x = Dates_test, y = Afinn), col="red") +
  geom_line(data=Af_pred_int_M,aes(x=date,y=lower_M_Af),col="red")+
  geom_line(data=Af_pred_int_M,aes(x=date,y=upper_M_Af),col="red")+
  ggtitle("Afinn score prediction interval with Mixed Copula") +
  xlab("Date") +
...
### Hospital cases
#
HC_pred_int_M<-as.data.frame(test_HC[1:37,])
HC_pred_int_M<-cbind(HC_pred_int_M,lower_M_HC,upper_M_HC)
#
UK_predintplot_M_HC <- ggplot(HC_pred_int_M,aes(x = date, y = hospitalCases)) + 
  geom_line() + 
  geom_line(data=UK_sim_avg, aes(x = Dates_test, y = Hospital_cases), col="red") +
  geom_line(data=HC_pred_int_M,aes(x=date,y=lower_M_HC),col="red",linetype=2)+
  geom_line(data=HC_pred_int_M,aes(x=date,y=upper_M_HC),col="red",linetype=2)+
  ggtitle("Hospital cases prediction interval with Mixed Copula") +
  xlab("Date") +
  ylab("Number of hospital cases")+
  theme(axis.text=element_text(size=20),
        axis.title=element_text(size=20),
...
### Occupied Beds
#
OB_pred_int_M<-as.data.frame(test_OB[1:37,])
OB_pred_int_M<-cbind(OB_pred_int_M,lower_M_OB,upper_M_OB)
#
UK_predintplot_M_OB <- ggplot(OB_pred_int_M,aes(x = date, y = covidOccupiedMVBeds)) + 
  geom_line() + 
  geom_line(data=UK_sim_avg, aes(x = Dates_test, y = Occupied_Beds), col="red") +
  geom_line(data=OB_pred_int_M,aes(x=date,y=lower_M_OB),col="red",linetype=2)+
  geom_line(data=OB_pred_int_M,aes(x=date,y=upper_M_OB),col="red",linetype=2)+
  ggtitle("Occupied Beds prediction interval with Mixed Copula") +
  xlab("Date") +
  ylab("Number of ocupied beds")+
  theme(axis.text=element_text(size=20),
        axis.title=element_text(size=20),
        title = element_text(size=18))
UK_predintplot_M_OB
...
### Deaths
#
Deaths_pred_int_M<-as.data.frame(test_deaths[1:37,])
Deaths_pred_int_M<-cbind(Deaths_pred_int_M,lower_M_Deaths,upper_M_Deaths)
#
UK_predintplot_M_Deaths <- ggplot(Deaths_pred_int_M,aes(x = date, y = deaths_count)) + 
  geom_line() + 
  geom_line(data=UK_sim_avg, aes(x = Dates_test, y = Deaths), col="red") +
  geom_line(data=Deaths_pred_int_M,aes(x=date,y=lower_M_Deaths),col="red",linetype=2)+
  geom_line(data=Deaths_pred_int_M,aes(x=date,y=upper_M_Deaths),col="red",linetype=2)+
  ggtitle("Number of deaths prediction interval with Mixed Copula") +
  xlab("Date") +
  ylab("Nymber of deaths")+
  theme(axis.text=element_text(size=20),
        axis.title=element_text(size=20),
        title = element_text(size=20))
UK_predintplot_M_Deaths

### New Cases
#
NC_pred_int_M<-as.data.frame(test_NC[1:37,])
NC_pred_int_M<-cbind(NC_pred_int_M,lower_M_NC,upper_M_NC)
#
UK_predintplot_M_NC <- ggplot(NC_pred_int_M,aes(x = date, y = Cases)) + 
  geom_line() + 
  geom_line(data=UK_sim_avg, aes(x = Dates_test, y = New_Cases), col="red") +
  geom_line(data=NC_pred_int_M,aes(x=date,y=lower_M_NC),col="red",linetype=2)+
  geom_line(data=NC_pred_int_M,aes(x=date,y=upper_M_NC),col="red",linetype=2)+
  ggtitle("New cases prediction interval with Mixed Copula") +
  xlab("Date") +
...
### Google trends
#
G_pred_int_G<-as.data.frame(test_Go[1:37,])
G_pred_int_G<-cbind(G_pred_int_G,lower_G_GO,upper_G_GO)
#
UK_predintplot_G_G <- ggplot(G_pred_int_G,aes(x = date, y = Google)) + 
  geom_line() + 
  geom_line(data=UK_sim_G_avg, aes(x = Dates_test, y = Google), col="red") +
  geom_line(data=G_pred_int_G,aes(x=date,y=lower_G_GO),col="red")+
  geom_line(data=G_pred_int_G,aes(x=date,y=upper_G_GO),col="red")+
  ggtitle("Google trends prediction interval with Gaussian Copula") +
  xlab("Date") +
  ylab("number of searches")
UK_predintplot_G_G
...
### Twitter
#
Tw_pred_int_G<-as.data.frame(test_Tw[1:37,])
Tw_pred_int_G<-cbind(Tw_pred_int_G,lower_G_TW,upper_G_TW)
#
UK_predintplot_G_Tw <- ggplot(Tw_pred_int_G,aes(x = date, y = Total_tweets)) + 
  geom_line() + 
  geom_line(data=UK_sim_G_avg, aes(x = Dates_test, y = Twitter), col="red") +
  geom_line(data=Tw_pred_int_G,aes(x=date,y=lower_G_TW),col="red")+
  geom_line(data=Tw_pred_int_G,aes(x=date,y=upper_G_TW),col="red")+
  ggtitle("Twitter search prediction interval with Gaussian Copula") +
  xlab("Date") +
  ylab("number of searches")
UK_predintplot_G_Tw
...
### Virus Tests
#
VT_pred_int_G<-as.data.frame(test_VT[1:37,])
VT_pred_int_G<-cbind(VT_pred_int_G,lower_G_VT,upper_G_VT)
#
UK_predintplot_G_VT <- ggplot(VT_pred_int_G,aes(x = date, y = VTadj)) + 
  geom_line() + 
  geom_line(data=UK_sim_avg, aes(x = Dates_test, y = Virus_Tests), col="red") +
  geom_line(data=VT_pred_int_G,aes(x=date,y=lower_G_VT),col="red",linetype=2)+
  geom_line(data=VT_pred_int_G,aes(x=date,y=upper_G_VT),col="red",linetype=2)+
  ggtitle("Virus Test (per 1000) prediction interval with Gaussian Copula") +
  xlab("Date") +
  ylab("Number of Virus Tests")+
...
### New Admissions
#
NA_pred_int_G<-as.data.frame(test_NA[1:37,])
NA_pred_int_G<-cbind(NA_pred_int_G,lower_G_NA,upper_G_NA)
#
UK_predintplot_G_NA <- ggplot(NA_pred_int_G,aes(x = date, y = newAdmissions)) + 
  geom_line() + 
  geom_line(data=UK_sim_avg, aes(x = Dates_test, y = New_Admissions), col="red") +
  geom_line(data=NA_pred_int_G,aes(x=date,y=lower_G_NA),col="red")+
  geom_line(data=NA_pred_int_G,aes(x=date,y=upper_G_NA),col="red")+
  ggtitle("New Admissions prediction interval with Gaussian Copula") +
  xlab("Date") +
  ylab("Number of New Admissions")+
  theme(axis.text=element_text(size=10),
        axis.title=element_text(size=12))
...
### Bing
#
Bi_pred_int_G<-as.data.frame(test_Bing[1:37,])
Bi_pred_int_G<-cbind(Bi_pred_int_G,lower_G_Bi,upper_G_Bi)
#
UK_predintplot_G_Bi <- ggplot(Bi_pred_int_G,aes(x = date, y = Bing)) + 
  geom_line() + 
  geom_line(data=UK_sim_avg, aes(x = Dates_test, y = Bing), col="red") +
  geom_line(data=Bi_pred_int_G,aes(x=date,y=lower_G_Bi),col="red")+
  geom_line(data=Bi_pred_int_G,aes(x=date,y=upper_G_Bi),col="red")+
  ggtitle("Bing score prediction interval with Gaussian Copula") +
  xlab("Date") +
  ylab("Bing sentiment score")
UK_predintplot_G_Bi
...
### Afinn
#
Af_pred_int_G<-as.data.frame(test_Afinn[1:37,])
Af_pred_int_G<-cbind(Af_pred_int_G,lower_G_Af,upper_G_Af)
#
UK_predintplot_G_Af <- ggplot(Af_pred_int_G,aes(x = date, y = Afinn)) + 
  geom_line() + 
  geom_line(data=UK_sim_avg, aes(x = Dates_test, y = Afinn), col="red") +
  geom_line(data=Af_pred_int_G,aes(x=date,y=lower_G_Af),col="red")+
  geom_line(data=Af_pred_int_G,aes(x=date,y=upper_G_Af),col="red")+
  ggtitle("Afinn score prediction interval with Gaussian Copula") +
  xlab("Date") +
  ylab("Afinn sentiment score")
UK_predintplot_G_Af
...
### Hospital cases
#
HC_pred_int_G<-as.data.frame(test_HC[1:37,])
HC_pred_int_G<-cbind(HC_pred_int_G,lower_G_HC,upper_G_HC)
#
UK_predintplot_G_HC <- ggplot(HC_pred_int_G,aes(x = date, y = hospitalCases)) + 
  geom_line() + 
  geom_line(data=UK_sim_avg, aes(x = Dates_test, y = Hospital_cases), col="red") +
  geom_line(data=HC_pred_int_G,aes(x=date,y=lower_G_HC),col="red")+
  geom_line(data=HC_pred_int_G,aes(x=date,y=upper_G_HC),col="red")+
  ggtitle("Number of Hospital cases prediction interval with Gaussian Copula") +
  xlab("Date") +
  ylab("Number of hospital cases")
UK_predintplot_G_HC
...
### Occupied Beds
#
OB_pred_int_G<-as.data.frame(test_OB[1:37,])
OB_pred_int_G<-cbind(OB_pred_int_G,lower_G_OB,upper_G_OB)
#
UK_predintplot_G_OB <- ggplot(OB_pred_int_G,aes(x = date, y = covidOccupiedMVBeds)) + 
  geom_line() + 
  geom_line(data=UK_sim_avg, aes(x = Dates_test, y = Occupied_Beds), col="red") +
  geom_line(data=OB_pred_int_G,aes(x=date,y=lower_G_OB),col="red")+
  geom_line(data=OB_pred_int_G,aes(x=date,y=upper_G_OB),col="red")+
  ggtitle("Occupied Beds prediction interval with Gaussian Copula") +
  xlab("Date") +
  ylab("Number of ocupied beds")
UK_predintplot_G_OB
...
### Deaths
#
Deaths_pred_int_G<-as.data.frame(test_deaths[1:37,])
Deaths_pred_int_G<-cbind(Deaths_pred_int_G,lower_G_Deaths,upper_G_Deaths)
#
UK_predintplot_G_Deaths <- ggplot(Deaths_pred_int_G,aes(x = date, y = deaths_count)) + 
  geom_line() + 
  geom_line(data=UK_sim_avg, aes(x = Dates_test, y = Deaths), col="red") +
  geom_line(data=Deaths_pred_int_G,aes(x=date,y=lower_G_Deaths),col="red")+
  geom_line(data=Deaths_pred_int_G,aes(x=date,y=upper_G_Deaths),col="red")+
  ggtitle("Number of deaths prediction interval with Gaussian Copula") +
  xlab("Date") +
  ylab("Nymber of deaths")
UK_predintplot_G_Deaths

### New Cases
#
NC_pred_int_G<-as.data.frame(test_NC[1:37,])
NC_pred_int_G<-cbind(NC_pred_int_G,lower_G_NC,upper_G_NC)
#
UK_predintplot_G_NC <- ggplot(NC_pred_int_G,aes(x = date, y = Cases)) + 
  geom_line() + 
  geom_line(data=UK_sim_avg, aes(x = Dates_test, y = New_Cases), col="red") +
  geom_line(data=NC_pred_int_G,aes(x=date,y=lower_G_NC),col="red")+
  geom_line(data=NC_pred_int_G,aes(x=date,y=upper_G_NC),col="red")+
  ggtitle("New cases prediction interval with Gaussian Copula") +
  xlab("Date") +
  ylab("Number of new cases")
UK_predintplot_G_NC
...
### Google trends
#
G_pred_int_I<-as.data.frame(test_Go[1:37,])
G_pred_int_I<-cbind(G_pred_int_I,lower_I_GO,upper_I_GO)
#
UK_predintplot_I_G <- ggplot(G_pred_int_I,aes(x = date, y = Google)) + 
  geom_line() + 
  geom_line(data=UK_sim_I_avg, aes(x = Dates_test, y = Google), col="red") +
  geom_line(data=G_pred_int_I,aes(x=date,y=lower_I_GO),col="red")+
  geom_line(data=G_pred_int_I,aes(x=date,y=upper_I_GO),col="red")+
  ggtitle("Google trends prediction interval with Independence Copula") +
  xlab("Date") +
  ylab("number of searches")
UK_predintplot_I_G
...
### Twitter
#
Tw_pred_int_I<-as.data.frame(test_Tw[1:37,])
Tw_pred_int_I<-cbind(Tw_pred_int_I,lower_I_TW,upper_I_TW)
#
UK_predintplot_I_Tw <- ggplot(Tw_pred_int_I,aes(x = date, y = Total_tweets)) + 
  geom_line() + 
  geom_line(data=UK_sim_I_avg, aes(x = Dates_test, y = Twitter), col="red") +
  geom_line(data=Tw_pred_int_I,aes(x=date,y=lower_I_TW),col="red")+
  geom_line(data=Tw_pred_int_I,aes(x=date,y=upper_I_TW),col="red")+
  ggtitle("Twitter search prediction interval with Independence Copula") +
  xlab("Date") +
  ylab("number of searches")
UK_predintplot_I_Tw
...
### Virus Tests
#
VT_pred_int_I<-as.data.frame(test_VT[1:37,])
VT_pred_int_I<-cbind(VT_pred_int_I,lower_I_VT,upper_I_VT)
#
UK_predintplot_I_VT <- ggplot(VT_pred_int_I,aes(x = date, y = VTadj)) + 
  geom_line() + 
  geom_line(data=UK_sim_avg, aes(x = Dates_test, y = Virus_Tests), col="red") +
  geom_line(data=VT_pred_int_I,aes(x=date,y=lower_I_VT),col="red")+
  geom_line(data=VT_pred_int_I,aes(x=date,y=upper_I_VT),col="red")+
  ggtitle("Virus Test (per 1000) prediction interval with Independence Copula") +
  xlab("Date") +
  ylab("Number of Virus Tests")+
  theme(axis.text=element_text(size=10),
        axis.title=element_text(size=12))
...
### New Admissions
#
NA_pred_int_I<-as.data.frame(test_NA[1:37,])
NA_pred_int_I<-cbind(NA_pred_int_I,lower_I_NA,upper_I_NA)
#
UK_predintplot_I_NA <- ggplot(NA_pred_int_I,aes(x = date, y = newAdmissions)) + 
  geom_line() + 
  geom_line(data=UK_sim_avg, aes(x = Dates_test, y = New_Admissions), col="red") +
  geom_line(data=NA_pred_int_I,aes(x=date,y=lower_I_NA),col="red")+
  geom_line(data=NA_pred_int_I,aes(x=date,y=upper_I_NA),col="red")+
  ggtitle("New Admissions prediction interval with Independence Copula") +
  xlab("Date") +
  ylab("Number of New Admissions")+
  theme(axis.text=element_text(size=10),
        axis.title=element_text(size=12))
...
### Bing
#
Bi_pred_int_I<-as.data.frame(test_Bing[1:37,])
Bi_pred_int_I<-cbind(Bi_pred_int_I,lower_I_Bi,upper_I_Bi)
#
UK_predintplot_I_Bi <- ggplot(Bi_pred_int_I,aes(x = date, y = Bing)) + 
  geom_line() + 
  geom_line(data=UK_sim_avg, aes(x = Dates_test, y = Bing), col="red") +
  geom_line(data=Bi_pred_int_I,aes(x=date,y=lower_I_Bi),col="red")+
  geom_line(data=Bi_pred_int_I,aes(x=date,y=upper_I_Bi),col="red")+
  ggtitle("Bing score prediction interval with Independence Copula") +
  xlab("Date") +
  ylab("Bing sentiment score")
UK_predintplot_I_Bi
...
### Afinn
#
Af_pred_int_I<-as.data.frame(test_Afinn[1:37,])
Af_pred_int_I<-cbind(Af_pred_int_I,lower_I_Af,upper_I_Af)
#
UK_predintplot_I_Af <- ggplot(Af_pred_int_I,aes(x = date, y = Afinn)) + 
  geom_line() + 
  geom_line(data=UK_sim_avg, aes(x = Dates_test, y = Afinn), col="red") +
  geom_line(data=Af_pred_int_I,aes(x=date,y=lower_I_Af),col="red")+
  geom_line(data=Af_pred_int_I,aes(x=date,y=upper_I_Af),col="red")+
  ggtitle("Afinn score prediction interval with Independence Copula") +
  xlab("Date") +
  ylab("Afinn sentiment score")
UK_predintplot_I_Af
...
### Hospital cases
#
HC_pred_int_I<-as.data.frame(test_HC[1:37,])
HC_pred_int_I<-cbind(HC_pred_int_I,lower_I_HC,upper_I_HC)
#
UK_predintplot_I_HC <- ggplot(HC_pred_int_I,aes(x = date, y = hospitalCases)) + 
  geom_line() + 
  geom_line(data=UK_sim_avg, aes(x = Dates_test, y = Hospital_cases), col="red") +
  geom_line(data=HC_pred_int_I,aes(x=date,y=lower_I_HC),col="red")+
  geom_line(data=HC_pred_int_I,aes(x=date,y=upper_I_HC),col="red")+
  ggtitle("Number of Hospital cases prediction interval with Independence Copula") +
  xlab("Date") +
  ylab("Number of hospital cases")
UK_predintplot_I_HC
...
### Occupied Beds
#
OB_pred_int_I<-as.data.frame(test_OB[1:37,])
OB_pred_int_I<-cbind(OB_pred_int_I,lower_I_OB,upper_I_OB)
#
UK_predintplot_I_OB <- ggplot(OB_pred_int_I,aes(x = date, y = covidOccupiedMVBeds)) + 
  geom_line() + 
  geom_line(data=UK_sim_avg, aes(x = Dates_test, y = Occupied_Beds), col="red") +
  geom_line(data=OB_pred_int_I,aes(x=date,y=lower_I_OB),col="red")+
  geom_line(data=OB_pred_int_I,aes(x=date,y=upper_I_OB),col="red")+
  ggtitle("Occupied Beds prediction interval with Independence Copula") +
  xlab("Date") +
  ylab("Number of ocupied beds")
UK_predintplot_I_OB
...
### Deaths
#
Deaths_pred_int_I<-as.data.frame(test_deaths[1:37,])
Deaths_pred_int_I<-cbind(Deaths_pred_int_I,lower_I_Deaths,upper_I_Deaths)
#
UK_predintplot_I_Deaths <- ggplot(Deaths_pred_int_I,aes(x = date, y = deaths_count)) + 
  geom_line() + 
  geom_line(data=UK_sim_avg, aes(x = Dates_test, y = Deaths), col="red") +
  geom_line(data=Deaths_pred_int_I,aes(x=date,y=lower_I_Deaths),col="red")+
  geom_line(data=Deaths_pred_int_I,aes(x=date,y=upper_I_Deaths),col="red")+
  ggtitle("Number of deaths prediction interval with Independence Copula") +
  xlab("Date") +
  ylab("Nymber of deaths")
UK_predintplot_I_Deaths

### New Cases
#
NC_pred_int_I<-as.data.frame(test_NC[1:37,])
NC_pred_int_I<-cbind(NC_pred_int_I,lower_I_NC,upper_I_NC)
#
UK_predintplot_I_NC <- ggplot(NC_pred_int_I,aes(x = date, y = Cases)) + 
  geom_line() + 
  geom_line(data=UK_sim_avg, aes(x = Dates_test, y = New_Cases), col="red") +
  geom_line(data=NC_pred_int_I,aes(x=date,y=lower_I_NC),col="red")+
  geom_line(data=NC_pred_int_I,aes(x=date,y=upper_I_NC),col="red")+
  ggtitle("New cases prediction interval with Independence Copula") +
  xlab("Date") +
  ylab("Number of new cases")
UK_predintplot_I_NC

# Tree 1
plot(RVM_pa,tree=1,edge_labels="family_tau",var_names="use")

plot(RVM_pa)

RVineTreePlot(RVM_pa,tree=1,edge.labels = C("family","theotau"))

plot(RVM_pa,tree=1,edge.labels = "family-tau",type=2)
